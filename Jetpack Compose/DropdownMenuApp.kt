...
fun MyDropdownMenu(text: MutableState<String>){
    ...
    DropdownMenu(
        ...
        DropdownMenuItem(
            text = { Text(text = "Обновить") },
            onClick = {text.value = ""},
            leadingIcon = { Icon(Icons.Rounded.Refresh, "Refresh") }
        )
        HorizontalDivider(thickness = 2.dp)
        DropdownMenuItem(
            text = { Text(text = "О создателе") },
            onClick = {alertDialogState.value = true},
            leadingIcon = { Icon(Icons.Rounded.Info, "About") }
        )
    }
    ...
    if (alertDialogState.value) {
        AlertDialog(
            ...
            title = { Text(text = "О создателе") },
            text = { Text(text = "Создал Я :)") },
            confirmButton = { Button(
                onClick = {alertDialogState.value = false}
            ) { Text(text = "Закрыть") } }
        )
    }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun MyTopBar(text: MutableState<String>){
    TopAppBar(
